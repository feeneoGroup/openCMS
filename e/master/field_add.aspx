...
<tr id="T_fckstyle">
  ...
  <td align="left"><input type="radio" name="f_fckstyle" value="Small" <%=Field_fckstyle=="Small"?"checked":""%>>精简版 <input type="radio" name="f_fckstyle" value="Simple"  <%=Field_fckstyle=="Simple"?"checked":""%>>简化版  <input type="radio" name="f_fckstyle" value="Normal" <%=Field_fckstyle=="Normal"?"checked":""%>>全功能版 </td>
 </tr>
...
<tr id="T_RelationTable">
  <td  height=25 align="right">关联数据表：</td>
  <td align="left"><select name="f_RelationTable" id="f_RelationTable"><option value="">无设置</option><%=TableList%></select> 
  <input type="radio"  name="f_Multiple_Choice" value="0" <%=Filed_multiple_choice!="1"?"checked":""%>>单选 <input type="radio"  name="f_Multiple_Choice" id="f_Multiple_Choice_1" value="1" <%=Filed_multiple_choice=="1"?"checked":""%>>多选

  <span style="color:#666666"> 选择后则可以关联对应表单的数据,并在字段自定义中生成对应的表单代码，可自行修改扩展代码</span>
</td>
</tr>
...
显示字数：<input name="f_listitem_words" id="f_listitem_words" type="text" size="4" maxlength="3" value="<%=Field_listitem_words==null?"50":Field_listitem_words%>"> <span style="color:#666">数值型和日期型字段不受限制</span>
 ...
 <tr id="T_7">
  ...
  <td align="left"><Textarea name="f_attribute" id="f_attribute" cols="50" rows="4" title="每行一个属性"><%=Field_Attribute%></Textarea></td>
 </tr>

 <tr id="T_js">
  <td  height=25 align="right">提交验证js：</td>
  <td align="left"><Textarea name="f_submit_js" id="f_submit_js" cols="50" rows="5" title="在点击提交按钮前进行验证"><%=Field_SubmitJs%></Textarea></td>
 </tr>
...
<tr>
  ...
  <td align="left">
    <table border="0" cellpadding="0" cellspacing="0" width=100%>
     <tr title="后台新增信息的自定义表单"><td width="80px">增加表单</td><td><Textarea name="master_form_add" id="master_form_add" cols="80" rows="6" ><%=Master_Form_Add%></Textarea></td></tr>
     <tr title="后台编辑信息的自定义表单"><td width="80px">编辑表单</td><td><Textarea name="master_form_edit" id="master_form_edit"  cols="80" rows="6" ><%=Master_Form_Edit%></Textarea></td></tr>
     </table>
</td>
</tr>
...
<tr>
  ...
  <td align="left">
     <table border="0" cellpadding="0" cellspacing="0" width=100%>
     <tr title="会员新增信息的自定义表单"><td width="80px">增加表单</td><td><Textarea name="member_form_add" id="member_form_add"  cols="80" rows="6" ><%=Member_Form_Add%></Textarea></td></tr>
     <tr title="会员编辑信息的自定义表单"><td width="80px">编辑表单</td><td><Textarea name="member_form_edit" id="member_form_edit"  cols="80" rows="6" ><%=Member_Form_Edit%></Textarea></td></tr>
     </table>
  </td>
</tr>
</table>
...
function InsertFormCode()
 {
   ...
   if(f_Multiple_Choice_1.checked)
    {
     IsMultipl=1;
    }
  if(field==""){return;}
  ...
  var add_form="<!--add-->\r\n<br><input type=\"button\" value=\"选择关联信息\" onclick='Data_Select(\""+table+"\",\""+field+"\","+IsMultipl+",\"选择关联信息\")'>"
  var edit_form="<!--add-->\r\n<select id=\""+field+"_data\" style=\"display:none\">\r\n<"+"%\r\nstring "+field+"=r(\""+field+"\");\r\nif("+field+"!=\"\")\r\n{\r\nDataTable dt=Get_Data(\"select id,title from "+table+" where id in(\"+"+field+"+\")\");\r\nDataRow dr;\r\nfor(int i=0;i<dt.Rows.Count;i++)\r\n {\r\n  dr=dt.Rows[i];\r\n%>\r\n<option value=\"<"+"%=dr[\"id\"]%"+">\"><"+"%=dr[\"title\"]%"+"></option>\r\n<"+"%}\r\n}%"+">\r\n</select>\r\n<s"+"cript type=\"text/javascript\">\r\nvar "+field+"=Id(\""+field+"\");\r\nvar "+field+"_data=Id(\""+field+"_data\");\r\nfor(var i=0; i<"+field+"_data.length; i++)\r\n{\r\n   "+field+".add(new Option("+field+"_data.options[i].text,"+field+"_data.options[i].value));\r\n}\r\n</"+"script>\r\n<br><input type=\"button\" value=\"选择关联信息\"  onclick='Data_Select(\""+table+"\",\""+field+"\","+IsMultipl+",\"选择关联信息\")'>";
